...
var backend = function(monitor) {
  ...
  try {
    self._driver = require('marklogic');
...
backend.prototype.setAdminDBSettings = function(settings) {
  this._adminSettings = {
    ...
    authType: settings.auth.toUpperCase()
  };
  this._dbAdmin = this._driver.createDatabaseClient(this._adminSettings);
  this._manager = ???.createManager(this._dbAdmin);
